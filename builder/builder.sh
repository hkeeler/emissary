...
case "${cmd}" in
    ...
    compile-internal)
        ...
        for MODDIR in $(find-modules); do
            ...
            eval "$(grep VERSION apro.version 2>/dev/null)" # this will `eval ''` for OSS-only builds, leaving VERSION unset; dont embed the version-number in OSS Go binaries
            ...
            if [ -e ${module}.dirty ] || ([ "$module" != ambassador ] && [ -e go.dirty ]) ; then
                if [ -e "${MODDIR}/go.mod" ]; then
                    ...
                    (cd ${MODDIR} && time go build -trimpath ${VERSION:+ -ldflags "-X main.Version=$VERSION" } -o /buildroot/bin ./cmd/...) || exit 1
